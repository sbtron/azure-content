...
1. Add the following to the *ServiceDefinition.csdef* file under the *WebRole* or *WorkerRole* node for all roles:
	...
	The above configuration will run the console command *install.cmd* with administrator privileges so it can install the .NET framework. The configuration also creates a LocalStorage with the name *InstallLogs* to store any log information created by the install script. For more see: [Use local storage to store files during startup](https://msdn.microsoft.com/en-us/library/azure/hh974419.aspx) 
...
This will configure azure diagnostics to transfer all files in the *InstallLogs* resource to the diagnostics storage account.
...
[Installing the .NET Framework]: https://msdn.microsoft.com/en-us/library/5a4x27ek(v=vs.110).aspx
[Troubleshooting .NET Framework Installations]: https://msdn.microsoft.com/en-us/library/hh925569(v=vs.110).aspx
